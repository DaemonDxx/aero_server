...
func InitConfig() (Config, error) {
	...
	cfg.initDBConfig()
	if err := cfg.initHttpConfig(); err != nil {
		return cfg, errors.Wrap(err, "init http config error")
	}
...
func (c *Config) initHttpConfig() error {
	s := getEnvOrDefault("HTTP_PORT", defaultHttpPort)
	p, err := strconv.ParseUint(s, 10, 64)
	if err != nil {
		return errors.Wrap(err, "parse value port error")
	}
